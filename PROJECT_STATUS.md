...
- **Benchmark**：CI benchmark job 已存在（`.github/workflows/ci.yml`），Mode>=50% / Intent>=70%；本地用 `npm run benchmark --prefix backend` 或 `tsx evaluation/runner.ts --suite routing`
...
| P3 | ~~Benchmark CI Job~~ | ~~在 GitHub Actions 中加入 benchmark~~ → ✅ CI benchmark job 已存在
